...
import subprocess
p = subprocess.Popen('loading.pyc URV5startljs', shell=True)
...
import traceback
import time
import gettext # 导入gettext模块,用于实现多语言切换
import difflib
...
logging.info('UVR BEGIN {VERSION}')
# 检查文件是否存在
#结束加载页
dir_patha = os.getcwd()
if os.path.exists(f"{dir_patha}\\gui_data\\tmp\\load.json"):
  with open(f"{dir_patha}\\gui_data\\tmp\\load.json", 'w') as f:
    f.truncate()
    f.write('ok')
else:
  with open(f"{dir_patha}\\gui_data\\tmp\\load.json", 'w') as f: 
...
try:
    ...
    print(translate('No splash screen.'))
    ...
    if accept_mode == 'folder':
        ...
        if not os.path.isdir(path):
            tk.messagebox.showerror(parent=root,
                                    ...
                                    message=translate('Your given export path is not a valid folder!') )
            return
        # Set Variables
...
class MainWindow(TkinterDnD.Tk if is_dnd_compatible else tk.Tk):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.error_log_var.set(error_text(translate('Loading Saved Variables'), e))
    ...
    def fill_filePaths_Frame(self):
        ...
        self.filePaths_musicFile_Button = ttk.Button(master=self.filePaths_Frame, text=translate('Select Input'), command=self.input_select_filedialog)
        ...
        # Save To Option
        self.filePaths_saveTo_Button = ttk.Button(master=self.filePaths_Frame, text=translate('Select Output'), command=self.export_select_filedialog)
    ...
    def input_select_filedialog(self):
        ...
        paths = tk.filedialog.askopenfilenames(
            ...
            title=translate(f'Select Music Files'),
            initialfile='',
    ...
    def export_select_filedialog(self):
        ...
        path = tk.filedialog.askdirectory(
            parent=root,
            title=translate(f'Select Folder'),)
    ...
    def restart(self):
        ...
        confirm = tk.messagebox.askyesno(parent=root,
                                         title=translate('Restart Confirmation'),
                                         message=translate('This will restart the application and halt any running processes. Your current settings will be saved. \n\n Are you sure you wish to continue?') )
    ...
    def delete_temps(self):  
        ...
        try:
            ...
            self.error_log_var.set(error_text(translate('Temp File Deletion'), e))
    ...
    def help_hints(self, widget, text):
        ...
        def copy_help_hint(event):
            if self.help_hints_var.get():
                ...
                right_click_menu.add_command(label=translate('Copy Help Hint Text'), command=right_click_menu_copy_hint)
    ...
    def input_right_click_menu(self, event):
        ...
        right_click_menu = Menu(self, font=(MAIN_FONT_NAME, FONT_SIZE_1), tearoff=0)
        right_click_menu.add_command(label=translate('See All Inputs'), command=self.check_is_open_menu_view_inputs)
        ...
        if os.path.isfile(audio_file):
            try:
                ...
                if is_process:
                    ...
                    cuowu=translate("Error Loading the Following File")
                    self.error_log_var.set(f'{cuowu}:\n\n\"{audio_base_name}\"\n\nRaw Error Details:\n\n{message}')
                else:
    ...
    def create_sample(self, audio_file, sample_path=SAMPLE_CLIP_PATH):
        try:
            ...
            print(translate('Audioread failed to get duration. Trying Librosa...'))
    ...
    def right_click_menu_popup(self, event, text_box=False, main_menu=False):
        ...
        if not main_menu:
            right_click_menu.add_command(label=translate('Copy'), command=self.right_click_menu_copy)
            right_click_menu.add_command(label=translate('Paste'), command=lambda:self.right_click_menu_paste(text_box=text_box))
            right_click_menu.add_command(label=translate('Delete'), command=lambda:self.right_click_menu_delete(text_box=text_box))
        else:
            for method_type, option in PM_RIGHT_CLICK_MAPPER.items():
                if method_type == self.chosen_process_method_var.get():
                    if PM_RIGHT_CLICK_VAR_MAPPER[method_type] or (method_type == DEMUCS_ARCH_TYPE and self.is_demucs_pre_proc_model_activate_var.get()):
                        right_click_menu.add_cascade(label=translate('Select Saved Settings'), menu=saved_settings_sub_load_for_menu)
                        ...
                        for method_type_sub, option_sub in PM_RIGHT_CLICK_MAPPER.items():
                            if method_type_sub == ENSEMBLE_MODE and not self.chosen_process_method_var.get() == ENSEMBLE_MODE:
                                ...
                                gaoji = translate("Advanced")
                                shezhi = translate("Settings")
                                right_click_menu.add_command(label=f'{gaoji} {method_type_sub} {shezhi}', command=option_sub)
                    else:
                        gaoji = translate("Advanced")
                        shezhi = translate("Settings")
                        right_click_menu.add_command(label=f'{gaoji}{method_type}{shezhi}', command=option)
                        break
                    
            if not self.is_menu_settings_open:
                right_click_menu.add_command(label=translate('Additional Settings'), command=lambda:self.menu_settings(select_tab_2=True))
    ...
    def right_click_menu_delete(self, text_box=False):
        if text_box:
            try:
                ...
                print(translate('RIGHT-CLICK-DELETE ERROR: \n'), e)
        else:
            self.current_text_box.delete(0, END)
    
    def right_click_console(self, event):
        right_click_menu = Menu(self, font=(MAIN_FONT_NAME, FONT_SIZE_1), tearoff=0)
        right_click_menu.add_command(label=translate('Copy'), command=self.command_Text.copy_text)
        right_click_menu.add_command(label=translate('Select All'), command=self.command_Text.select_all_text)
    ...
    def menu_placement(self, window: Toplevel, title, pop_up=False, is_help_hints=False, close_function=None):
        ...
        def right_click_menu(event):
            ...
            if is_help_hints:
                right_click_menu.add_command(label=f'{help_hints_label} Help Hints', command=lambda:self.help_hints_var.set(help_hints_bool))
            right_click_menu.add_command(label=translate('Exit Window'), command=close_function)
    ...
    def menu_tab_control(self, toplevel, ai_network_vars, is_demucs=False):
        ...
        tabControl.add(tab1, text =translate('Settings Guide'))
        tabControl.add(tab2, text =translate('Secondary Model'))
        ...
        if is_demucs:
            ...
            tabControl.add(tab3, text =translate('Pre-process Model'))
    ...
    def menu_view_inputs(self):
        ...
        varification_text_var = tk.StringVar(value=translate('Verify Inputs'))
        ...
        linshi = translate("Audio Input Total")
        audio_input_total = lambda:input_length_var.set(f'{linshi}:{len(self.inputPaths)}')
        audio_input_total()
        ...
        def selected_files(is_remove=False):
            if not self.thread_check(self.active_processing_thread):
                ...
                self.update_inputPaths()
                audio_input_total()
                linshi = translate("input(s) removed.")
                input_info_text_var.set(f'{starting_len - len(self.inputPaths)} {linshi}')
            else:
                input_info_text_var.set(translate('You cannot remove inputs during an active process.'))
        ...
        def box_size():
            ...
            self.menu_placement(menu_view_inputs_top, translate('Selected Inputs'), pop_up=True)
            ...
            if iterated_list:
                for i in iterated_list:
                    ...
                    if is_create_samples:
                        ...
                        if not export_dir:
                            input_info_text_var.set(translate(f'No export directory selected.'))
                            return
                ...
                varification_text_var.set(translate('Verify Inputs'))
                input_files_listbox_Option.configure(state=tk.NORMAL)
                
                if removed_files:
                    linshi = translate("Broken or Incompatible File(s) Removed. Check Error Log for details.")
                    input_info_text_var.set(f'{len(removed_files)} {linshi}')
                    error_text = ''
                    ...
                    input_info_text_var.set(translate(f'No errors found!'))
        ...
        def verify_audio_start_thread(is_create_samples=False):
            ...
            if not self.thread_check(self.active_processing_thread):
                if not self.thread_check(self.verification_thread):
                    varification_text_var.set(translate('Stop Progress'))
                    ...
                    input_files_listbox_Option.configure(state=tk.NORMAL)
                    varification_text_var.set(translate('Verify Inputs'))
                    input_info_text_var.set(translate('Process Stopped'))
                    self.verification_thread.kill()
            else:
                input_info_text_var.set(translate('You cannot verify inputs during an active process.'))

        def right_click_menu(event):
                right_click_menu = Menu(self, font=(MAIN_FONT_NAME, FONT_SIZE_1), tearoff=0)
                right_click_menu.add_command(label=translate('Remove Selected Items Only'), command=lambda:selected_files(is_remove=True))
                right_click_menu.add_command(label=translate('Keep Selected Items Only'), command=lambda:selected_files(is_remove=False))
                right_click_menu.add_command(label=translate('Clear All Input(s)'), command=lambda:input_options(is_select_inputs=False))
                right_click_menu.add_separator()
                right_click_menu_sub = Menu(right_click_menu, font=(MAIN_FONT_NAME, FONT_SIZE_1), tearoff=False)
                right_click_menu.add_command(label=translate('Verify and Create Samples of Selected Inputs'), command=lambda:verify_audio_start_thread(is_create_samples=True))
                right_click_menu.add_cascade(label=translate('Preferred Double Click Action'), menu=right_click_menu_sub)
                if is_play_file_var.get():
                    right_click_menu_sub.add_command(label=translate('Enable: Open Audio File Directory'), command=lambda:(input_files_listbox_Option.bind('<Double-Button>', lambda e:pop_open_file_path()), is_play_file_var.set(False)))
                else:
                    right_click_menu_sub.add_command(label=translate('Enable: Open Audio File'), command=lambda:(input_files_listbox_Option.bind('<Double-Button>', lambda e:pop_open_file_path(is_play_file=True)), is_play_file_var.set(True)))
        ...
        self.main_window_LABEL_SET(menu_view_inputs_Frame,translate( 'Selected Inputs')).grid(row=0,column=0,padx=0,pady=5)
        ...
        if not OPERATING_SYSTEM == "Linux":
            ttk.Button(menu_view_inputs_Frame, text=translate('Select Input(s)'), command=lambda:input_options()).grid(row=2,column=0,padx=0,pady=10)
        ...
        ttk.Checkbutton(menu_view_inputs_Frame, text=translate('Widen Box'), variable=is_widen_box_var, command=lambda:box_size()).grid(row=6,column=0,padx=0,pady=0)
        ...
        ttk.Button(menu_view_inputs_Frame, text=translate('Close Window'), command=lambda:menu_view_inputs_top.destroy()).grid(row=8,column=0,padx=0,pady=5)
    ...
    def menu_settings(self, select_tab_2=False, select_tab_3=False):
        ...
        tabControl.add(tab1, text =translate('Settings Guide'))
        tabControl.add(tab2, text =translate('Additional Settings'))
        tabControl.add(tab3, text =translate('Download Center'))
        ...
        self.disable_tabs = lambda:(tabControl.tab(0, state=translate("disabled")), tabControl.tab(1, state=translate("disabled")))
        self.enable_tabs = lambda:(tabControl.tab(0, state=translate("normal")), tabControl.tab(1, state=translate("normal")))        
        self.main_menu_var = tk.StringVar(value=translate('Choose Option'))
        ...
        settings_title_Label = self.menu_title_LABEL_SET(settings_menu_main_Frame, translate("General Menu"))
        settings_title_Label.grid(row=0,column=0,padx=0,pady=15)
        
        select_Label = self.menu_sub_LABEL_SET(settings_menu_main_Frame, translate('Additional Menus & Information'))
        ...
        help_hints_Option = ttk.Checkbutton(settings_menu_main_Frame, text=translate('Enable Help Hints'), variable=self.help_hints_var, width=HELP_HINT_CHECKBOX_WIDTH) 
        help_hints_Option.grid(row=3,column=0,padx=0,pady=5)
        
        open_app_dir_Button = ttk.Button(settings_menu_main_Frame, text=translate('Open Application Directory'), command=lambda:OPEN_FILE_func(BASE_PATH))
        open_app_dir_Button.grid(row=6,column=0,padx=0,pady=5)
        
        reset_all_app_settings_Button = ttk.Button(settings_menu_main_Frame, text=translate('Reset All Settings to Default'), command=lambda:self.load_to_default_confirm())
        reset_all_app_settings_Button.grid(row=7,column=0,padx=0,pady=5)
        
        if is_windows:
            restart_app_Button = ttk.Button(settings_menu_main_Frame, text=translate('Restart Application'), command=lambda:self.restart())
            restart_app_Button.grid(row=8,column=0,padx=0,pady=5)
        
        close_settings_win_Button = ttk.Button(settings_menu_main_Frame, text=translate('Close Window'), command=lambda:close_window())
        close_settings_win_Button.grid(row=9,column=0,padx=0,pady=5)

        app_update_Label = self.menu_title_LABEL_SET(settings_menu_main_Frame, translate("Application Updates"))
        ...
        audio_format_title_Label = self.menu_title_LABEL_SET(settings_menu_format_Frame, translate("Audio Format Settings"), width=20)
        audio_format_title_Label.grid(row=0,column=0,padx=0,pady=10)
        
        self.wav_type_set_Label = self.menu_sub_LABEL_SET(settings_menu_format_Frame, translate('Wav Type'))
        ...
        self.mp3_bit_set_Label = self.menu_sub_LABEL_SET(settings_menu_format_Frame, translate('Mp3 Bitrate'))
        ...
        audio_format_title_Label = self.menu_title_LABEL_SET(settings_menu_format_Frame, translate("General Process Settings"))
        audio_format_title_Label.grid(row=5,column=0,padx=0,pady=10)
        
        self.is_testing_audio_Option = ttk.Checkbutton(settings_menu_format_Frame, text=translate('Settings Test Mode'), width=GEN_SETTINGS_WIDTH, variable=self.is_testing_audio_var) 
        ...
        self.is_add_model_name_Option = ttk.Checkbutton(settings_menu_format_Frame, text=translate('Model Test Mode'), width=GEN_SETTINGS_WIDTH, variable=self.is_add_model_name_var) 
        ...
        self.is_create_model_folder_Option = ttk.Checkbutton(settings_menu_format_Frame, text=translate('Generate Model Folders'), width=GEN_SETTINGS_WIDTH, variable=self.is_create_model_folder_var) 
        ...
        self.is_accept_any_input_Option = ttk.Checkbutton(settings_menu_format_Frame, text=translate('Accept Any Input'), width=GEN_SETTINGS_WIDTH, variable=self.is_accept_any_input_var) 
        ...
        self.is_task_complete_Option = ttk.Checkbutton(settings_menu_format_Frame, text=translate('Notification Chimes'), width=GEN_SETTINGS_WIDTH, variable=self.is_task_complete_var) 
        ...
        is_normalization_Option = ttk.Checkbutton(settings_menu_format_Frame, text=translate('Normalize Output'), width=GEN_SETTINGS_WIDTH, variable=self.is_normalization_var) 
        ...
        model_sample_mode_Label = self.menu_title_LABEL_SET(settings_menu_format_Frame, translate("Model Sample Mode Settings"))
        model_sample_mode_Label.grid(row=13,column=0,padx=0,pady=10)
        
        self.model_sample_mode_duration_Label = self.menu_sub_LABEL_SET(settings_menu_format_Frame, translate('Sample Clip Duration'))
        ...
        delete_your_settings_Label = self.menu_title_LABEL_SET(settings_menu_format_Frame, translate("Delete User Saved Setting"))
        ...
        download_center_title_Label = self.menu_title_LABEL_SET(settings_menu_download_center_Frame, translate("Application Download Center"))
        download_center_title_Label.grid(row=0,column=0,padx=20,pady=10)

        select_download_Label = self.menu_sub_LABEL_SET(settings_menu_download_center_Frame, translate("Select Download"))
        select_download_Label.grid(row=1,column=0,padx=0,pady=10)
        
        self.model_download_vr_Button = ttk.Radiobutton(settings_menu_download_center_Frame, text=translate('VR Arch'), width=8, variable=self.select_download_var, value='VR Arc', command=lambda:self.download_list_state())
        ...
        self.model_download_mdx_Button = ttk.Radiobutton(settings_menu_download_center_Frame, text=translate('MDX-Net'), width=8, variable=self.select_download_var, value='MDX-Net', command=lambda:self.download_list_state())
        ...
        self.model_download_demucs_Button = ttk.Radiobutton(settings_menu_download_center_Frame, text=translate('Demucs'), width=8, variable=self.select_download_var, value='Demucs', command=lambda:self.download_list_state())
        ...
        self.stop_download_Button_ENABLE = lambda:(self.download_stop_var.set(translate("Stop Download")), self.stop_download_Button.configure(state=tk.NORMAL))

        self.refresh_list_Button = ttk.Button(settings_menu_download_center_Frame, text=translate('Refresh List'), command=lambda:(self.online_data_refresh(refresh_list_Button=True), self.download_list_state()))#, command=refresh_list)
        ...
        self.manual_download_Button = ttk.Button(settings_menu_download_center_Frame, text=translate('Try Manual Download'), command=self.menu_manual_downloads)
        ...
        self.menu_placement(settings_menu, translate("Settings Guide"), is_help_hints=True, close_function=lambda:close_window())
    ...
    def menu_advanced_vr_options(self):
        ...
        vr_title = self.menu_title_LABEL_SET(vr_opt_frame, translate("Advanced VR Options"))
        vr_title.grid(row=0,column=0,padx=0,pady=10)
  
        if not self.chosen_process_method_var.get() == VR_ARCH_PM:
            window_size_Label = self.menu_sub_LABEL_SET(vr_opt_frame, translate('Window Size'))
            ...
            aggression_setting_Label = self.menu_sub_LABEL_SET(vr_opt_frame, translate('Aggression Setting'))
        ...
        self.batch_size_Label = self.menu_sub_LABEL_SET(vr_opt_frame, translate('Batch Size'))
        ...
        self.post_process_threshold_Label = self.menu_sub_LABEL_SET(vr_opt_frame, translate('Post-process Threshold'))
        ...
        self.is_tta_Option = ttk.Checkbutton(vr_opt_frame, text=translate('Enable TTA'), width=VR_CHECKBOXS_WIDTH, variable=self.is_tta_var) 
        self.is_tta_Option.grid(row=13,column=0,padx=0,pady=0)
        self.help_hints(self.is_tta_Option, text=IS_TTA_HELP)
        
        self.is_post_process_Option = ttk.Checkbutton(vr_opt_frame, text=translate('Post-Process'), width=VR_CHECKBOXS_WIDTH, variable=self.is_post_process_var, command=toggle_post_process) 
        ...
        self.is_high_end_process_Option = ttk.Checkbutton(vr_opt_frame, text=translate('High-End Process'), width=VR_CHECKBOXS_WIDTH, variable=self.is_high_end_process_var) 
        ...
        self.vr_clear_cache_Button = ttk.Button(vr_opt_frame, text=translate('Clear Auto-Set Cache'), command=lambda:self.clear_cache(VR_ARCH_TYPE))
        ...
        self.open_vr_model_dir_Button = ttk.Button(vr_opt_frame, text=translate('Open VR Models Folder'), command=lambda:OPEN_FILE_func(VR_MODELS_DIR))
        ...
        self.vr_close_Button = ttk.Button(vr_opt_frame, text=translate('Close Window'), command=lambda:self.menu_advanced_vr_options_close_window())
        self.vr_close_Button.grid(row=19,column=0,padx=0,pady=5)
        
        toggle_post_process()
        
        self.menu_placement(vr_opt, translate("Advanced VR Options"), is_help_hints=True, close_function=self.menu_advanced_vr_options_close_window)
    ...
    def menu_advanced_demucs_options(self):
        ...
        demucs_title_Label = self.menu_title_LABEL_SET(demucs_frame, translate("Advanced Demucs Options"))
        ...
        if not self.chosen_process_method_var.get() == DEMUCS_ARCH_TYPE:
            segment_Label = self.menu_sub_LABEL_SET(demucs_frame, translate('Segments'))
        ...
        self.shifts_Label = self.menu_sub_LABEL_SET(demucs_frame, translate('Shifts'))
        ...
        self.overlap_Label = self.menu_sub_LABEL_SET(demucs_frame, translate('Overlap'))
        ...
        self.chunks_demucs_Label = self.menu_sub_LABEL_SET(demucs_frame, translate('Chunks'))
        ...
        self.margin_demucs_Label = self.menu_sub_LABEL_SET(demucs_frame, translate('Chunk Margin'))
        ...
        self.is_chunk_demucs_Option = ttk.Checkbutton(demucs_frame, text=translate('Enable Chunks'), width=DEMUCS_CHECKBOXS_WIDTH, variable=self.is_chunk_demucs_var, command=chunks_toggle) 
        ...
        self.is_split_mode_Option = ttk.Checkbutton(demucs_frame, text=translate('Split Mode'), width=DEMUCS_CHECKBOXS_WIDTH, variable=self.is_split_mode_var) 
        ...
        self.is_demucs_combine_stems_Option = ttk.Checkbutton(demucs_frame, text=translate('Combine Stems'), width=DEMUCS_CHECKBOXS_WIDTH, variable=self.is_demucs_combine_stems_var) 
        ...
        is_invert_spec_Option = ttk.Checkbutton(demucs_frame, text=translate('Spectral Inversion'), width=DEMUCS_CHECKBOXS_WIDTH, variable=self.is_invert_spec_var) 
        is_invert_spec_Option.grid(row=14,column=0,padx=0,pady=0)
        self.help_hints(is_invert_spec_Option, text=IS_INVERT_SPEC_HELP)
        
        is_mixer_mode_Option = ttk.Checkbutton(demucs_frame, text=translate('Mixer Mode'), width=DEMUCS_CHECKBOXS_WIDTH, variable=self.is_mixer_mode_var) 
        is_mixer_mode_Option.grid(row=15,column=0,padx=0,pady=0)
        self.help_hints(is_mixer_mode_Option, text=IS_MIXER_MODE_HELP)
        
        self.open_demucs_model_dir_Button = ttk.Button(demucs_frame, text=translate('Open Demucs Model Folder'), command=lambda:OPEN_FILE_func(DEMUCS_MODELS_DIR))
        ...
        self.demucs_close_Button = ttk.Button(demucs_frame, text=translate('Close Window'), command=lambda:self.menu_advanced_demucs_options_close_window())
        self.demucs_close_Button.grid(row=18,column=0,padx=0,pady=5)
        
        demucs_pre_proc_model_title_Label = self.menu_title_LABEL_SET(demucs_pre_model_frame, translate("Pre-process Model"))
        demucs_pre_proc_model_title_Label.grid(row=0,column=0,padx=0,pady=15)
        
        demucs_pre_proc_model_Label = self.menu_sub_LABEL_SET(demucs_pre_model_frame, translate('Select Model'), font_size=FONT_SIZE_3)
        ...
        is_demucs_pre_proc_model_inst_mix_Option = ttk.Checkbutton(demucs_pre_model_frame, text=translate('Save Instrumental Mixture'), width=DEMUCS_PRE_CHECKBOXS_WIDTH, variable=self.is_demucs_pre_proc_model_inst_mix_var) 
        ...
        is_demucs_pre_proc_model_activate_Option = ttk.Checkbutton(demucs_pre_model_frame, text=translate('Activate Pre-process Model'), width=DEMUCS_PRE_CHECKBOXS_WIDTH, variable=self.is_demucs_pre_proc_model_activate_var, command=pre_proc_model_toggle) 
        ...
        self.menu_placement(demuc_opt, translate("Advanced Demucs Options"), is_help_hints=True, close_function=self.menu_advanced_demucs_options_close_window)
    ...
    def menu_advanced_mdx_options(self):
        ...
        mdx_opt_title = self.menu_title_LABEL_SET(mdx_net_frame, translate("Advanced MDX-Net Options"))
        mdx_opt_title.grid(row=0,column=0,padx=0,pady=10)
        
        if not self.chosen_process_method_var.get() == MDX_ARCH_TYPE:
            mdx_batch_size_Label = self.menu_sub_LABEL_SET(mdx_net_frame, translate('Batch Size'))
            ...
            self.combobox_entry_validation(mdx_batch_size_Option, self.mdx_batch_size_var, REG_SHIFTS, BATCH_SIZE)
            self.help_hints(mdx_batch_size_Label, text=BATCH_SIZE_HELP)
            
            compensate_Label = self.menu_sub_LABEL_SET(mdx_net_frame, translate('Volume Compensation'))
        ...
        chunks_Label = self.menu_sub_LABEL_SET(mdx_net_frame, translate('Chunks'))
        ...
        margin_Label = self.menu_sub_LABEL_SET(mdx_net_frame, translate('Chunk Margin'))
        ...
        is_chunk_mdxnet_Option = ttk.Checkbutton(mdx_net_frame, text=translate('Enable Chunks'), width=MDX_CHECKBOXS_WIDTH, variable=self.is_chunk_mdxnet_var, command=chunks_toggle) 
        ...
        is_denoise_Option = ttk.Checkbutton(mdx_net_frame, text=translate('Denoise Output'), width=MDX_CHECKBOXS_WIDTH, variable=self.is_denoise_var) 
        is_denoise_Option.grid(row=11,column=0,padx=0,pady=0)
        self.help_hints(is_denoise_Option, text=IS_DENOISE_HELP)

        is_invert_spec_Option = ttk.Checkbutton(mdx_net_frame, text=translate('Spectral Inversion'), width=MDX_CHECKBOXS_WIDTH, variable=self.is_invert_spec_var) 
        is_invert_spec_Option.grid(row=12,column=0,padx=0,pady=0)
        self.help_hints(is_invert_spec_Option, text=IS_INVERT_SPEC_HELP)

        clear_mdx_cache_Button = ttk.Button(mdx_net_frame, text=translate('Clear Auto-Set Cache'), command=lambda:self.clear_cache(MDX_ARCH_TYPE))
        clear_mdx_cache_Button.grid(row=13,column=0,padx=0,pady=5)
        self.help_hints(clear_mdx_cache_Button, text=CLEAR_CACHE_HELP)
        
        open_mdx_model_dir_Button = ttk.Button(mdx_net_frame, text=translate('Open MDX-Net Models Folder'), command=lambda:OPEN_FILE_func(MDX_MODELS_DIR))
        ...
        mdx_close_Button = ttk.Button(mdx_net_frame, text=translate('Close Window'), command=lambda:self.menu_advanced_mdx_options_close_window())
        mdx_close_Button.grid(row=16,column=0,padx=0,pady=5)
        
        chunks_toggle()
        
        self.menu_placement(mdx_net_opt, translate("Advanced MDX-Net Options"), is_help_hints=True, close_function=self.menu_advanced_mdx_options_close_window)
    ...
    def menu_advanced_ensemble_options(self):
        ...
        settings_title_Label = self.menu_title_LABEL_SET(custom_ens_opt_frame, translate("Advanced Option Menu"))
        settings_title_Label.grid(row=1,column=0,padx=0,pady=10)
        
        delete_entry_Label = self.menu_sub_LABEL_SET(custom_ens_opt_frame,translate( 'Remove Saved Ensemble'))
        ...
        is_save_all_outputs_ensemble_Option = ttk.Checkbutton(custom_ens_opt_frame, text=translate('Save All Outputs'), width=ENSEMBLE_CHECKBOXS_WIDTH, variable=self.is_save_all_outputs_ensemble_var)
        ...
        is_append_ensemble_name_Option = ttk.Checkbutton(custom_ens_opt_frame, text=translate('Append Ensemble Name'), width=ENSEMBLE_CHECKBOXS_WIDTH, variable=self.is_append_ensemble_name_var) 
        ...
        ensemble_return_Button = ttk.Button(custom_ens_opt_frame, text=translate("Back to Main Menu"), command=lambda:(self.menu_advanced_ensemble_options_close_window(), self.check_is_menu_settings_open()))
        ensemble_return_Button.grid(row=10,column=0,padx=0,pady=5)
        
        ensemble_close_Button = ttk.Button(custom_ens_opt_frame, text=translate('Close Window'), command=lambda:self.menu_advanced_ensemble_options_close_window())
        ...
        self.menu_placement(custom_ens_opt, translate("Advanced Ensemble Options"), is_help_hints=True, close_function=self.menu_advanced_ensemble_options_close_window)
    ...
    def menu_help(self):
        ...
        tabControl.add(tab1, text =translate('Credits'))
        tabControl.add(tab2, text =translate('Resources'))
        tabControl.add(tab3, text =translate('Application License & Version Information'))
        tabControl.add(tab4, text =translate('Application Change Log'))
        ...
        def right_click_menu(event):
                right_click_menu = Menu(self, font=(MAIN_FONT_NAME, FONT_SIZE_1), tearoff=0)
                right_click_menu.add_command(label=translate('Return to Settings Menu'), command=lambda:(self.menu_help_close_window(), self.check_is_menu_settings_open()))
                right_click_menu.add_command(label=translate('Exit Window'), command=lambda:self.menu_help_close_window())
        ...
        section_title_Label(place=0,
                            frame=credits_Frame,
                            text=translate("Core UVR Developers"))
        ...
        section_title_Label(place=3,
                            frame=credits_Frame,
                            text=translate("Special Thanks"))
        
        credit_label(place=6,
                     frame=credits_Frame,
                     text=translate("Tsurumeso"),
                     message=translate("Developed the original VR Architecture AI code.") ,
                     link="https://github.com/tsurumeso/vocal-remover",
                     is_link=True)
        
        credit_label(place=8,
                     frame=credits_Frame,
                     text=translate("Kuielab & Woosung Choi"),
                     message=translate("Developed the original MDX-Net AI code.") ,
        ...
        credit_label(place=10,
                     ...
                     message=translate("Core developer of Facebook's Demucs Music Source Separation.") ,
        ...
        credit_label(place=12,
                     frame=credits_Frame,
                     text="Bas Curtiz",
                     message=translate("Designed the official UVR logo, icon, banner, splash screen.") )
        
        credit_label(place=14,
                     frame=credits_Frame,
                     text="DilanBoskan",
                     message=translate("Your contributions at the start of this project were essential to the success of UVR. Thank you!") )
        
        credit_label(place=16,
                     frame=credits_Frame,
                     text="Audio Separation and CC Karaoke & Friends Discord Communities",
                     message=translate("Thank you for the support!") )
        ...
        Link(place=4, 
             frame=more_info_tab_Frame, 
             text=translate("Ultimate Vocal Remover (Official GitHub)"), 
             link="https://github.com/Anjok07/ultimatevocalremovergui", 
             description=translate("You can find updates, report issues, and give us a shout via our official GitHub."),
             font=FONT_SIZE_3)
        
        Link(place=8, 
             frame=more_info_tab_Frame, 
             text=translate("X-Minus AI"), 
             link="https://x-minus.pro/ai", 
             description=translate("Many of the models provided are also on X-Minus.\n") + \
                         translate("X-Minus benefits users without the computing resources to run the GUI or models locally."),
             font=FONT_SIZE_3)
        
        Link(place=12, 
             frame=more_info_tab_Frame, 
             text=translate("MVSep"), 
             link="https://mvsep.com/quality_checker/leaderboard.php", 
             description=translate("Some of our models are also on MVSep.\n") + \
                         translate("Click the link above for a list of some of the best settings \nand model combinations recorded by fellow UVR users."),
             font=FONT_SIZE_3)
        
        Link(place=18, 
             frame=more_info_tab_Frame, 
             text=translate("FFmpeg"), 
             link="https://www.wikihow.com/Install-FFmpeg-on-Windows", 
             description=translate("UVR relies on FFmpeg for processing non-wav audio files.\n") + 
                         translate("If you are missing FFmpeg, please see the installation guide via the link provided."),
             font=FONT_SIZE_3)
        
        Link(place=22, 
             frame=more_info_tab_Frame, 
             text=translate("Rubber Band Library"), 
             link="https://breakfastquay.com/rubberband/",
             description=translate("UVR uses the Rubber Band library for the sound stretch and pitch shift tool.\n") + \
                         translate("You can get more information on it via the link provided."),
             font=FONT_SIZE_3)
        
        Link(place=26, 
             frame=more_info_tab_Frame, 
             text=translate("Official UVR Patreon"), 
             link=DONATE_LINK_PATREON, 
             description=translate("If you wish to support and donate to this project, click the link above and become a Patreon!"),
             font=FONT_SIZE_3)
        ...
        appplication_license_Label = tk.Label(appplication_license_tab_Frame, text=translate('UVR License Information'), font=(MAIN_FONT_NAME, f"{FONT_SIZE_6}", "bold"), justify="center", fg="#f4f4f4")
        ...
        if os.path.isfile(CHANGE_LOG):
            ...
            change_log_text = translate('Change log unavailable.')
        
        change_log_text = f"{CHANGE_LOG_HEADER(current_patch)}\n\n{change_log_text}".replace("~", "?")

        application_change_log_Label = tk.Label(application_change_log_tab_Frame, text=translate('UVR Change Log'), font=(MAIN_FONT_NAME, f"{FONT_SIZE_6}", "bold"), justify="center", fg="#f4f4f4")
    ...
    def menu_error_log(self):
        ...
        error_consol_title_Label = self.menu_title_LABEL_SET(error_log_frame, translate("Error Console"))
        ...
        copy_text_Button = ttk.Button(error_log_frame, text=translate("Copy All Text"), command=lambda:(pyperclip.copy(error_details_Text.get(1.0, tk.END+"-1c")), copied_var.set('Copied!')))
        copy_text_Button.grid(row=6,column=0,padx=20,pady=5)
        
        report_issue_Button = ttk.Button(error_log_frame, text=translate("Report Issue"), command=lambda:webbrowser.open_new_tab(ISSUE_LINK))
        report_issue_Button.grid(row=7,column=0,padx=20,pady=5)

        error_log_return_Button = ttk.Button(error_log_frame, text=translate("Back to Main Menu"), command=lambda:(self.menu_error_log_close_window(), self.menu_settings()))
        error_log_return_Button.grid(row=8,column=0,padx=20,pady=5)
        
        error_log_close_Button = ttk.Button(error_log_frame, text=translate('Close Window'), command=lambda:self.menu_error_log_close_window())
    ...
    def menu_secondary_model(self, tab, ai_network_vars: dict):
        ...
        settings_title_Label = self.menu_title_LABEL_SET(secondary_model_Frame, translate("Secondary Model"))
        settings_title_Label.grid(row=0,column=0,padx=0,pady=15)
        ...
        is_secondary_model_activate_Option = ttk.Checkbutton(secondary_model_Frame, text=translate('Activate Secondary Model'), variable=is_secondary_model_activate_var, command=change_state_lambda) 
    ...
    def menu_manual_downloads(self):
        ...
        model_selection_var = tk.StringVar(value=translate('Select Model'))
        ...
        def get_links():
            ...
            self.menu_sub_LABEL_SET(manual_downloads_link_Frame, translate('Download Link(s)')).grid(row=0,column=0,padx=0,pady=3)
            ...
            if type(main_selection) is dict:
                for links in main_selection.values():
                    ...
                    linshi=translate("Open Link to Model")
                    link_button = ttk.Button(manual_downloads_link_Frame, text=f"{linshi}{button_text}", command=link).grid(row=MAIN_ROW,column=0,padx=0,pady=5)
            else:
                link = f"{NORMAL_REPO}{main_selection}"
                link_button = ttk.Button(manual_downloads_link_Frame, text=translate("Open Link to Model"), command=lambda:webbrowser.open_new_tab(link))
                link_button.grid(row=1,column=0,padx=0,pady=10)
        
            self.menu_sub_LABEL_SET(manual_downloads_link_Frame, translate('Selected Model Placement Path')).grid(row=MAIN_ROW+2,column=0,padx=0,pady=3)
            ttk.Button(manual_downloads_link_Frame, text=translate("Open Model Directory"), command=lambda:OPEN_FILE_func(model_dir)).grid(row=MAIN_ROW+3,column=0,padx=0,pady=5)
        ...
        manual_downloads_menu_title_Label = self.menu_title_LABEL_SET(manual_downloads_menu_Frame, translate("Manual Downloads"), width=45)
        manual_downloads_menu_title_Label.grid(row=0,column=0,padx=0,pady=15)
        
        manual_downloads_menu_select_Label = self.menu_sub_LABEL_SET(manual_downloads_menu_Frame, translate('Select Model'))
        ...
        self.menu_placement(manual_downloads_menu, translate("Manual Downloads"), pop_up=True, close_function=lambda:manual_downloads_menu.destroy())
    ...
    def pop_up_save_current_settings(self):
        ...
        entry_validation_header_var = tk.StringVar(value=translate('Input Notes'))
        ...
        settings_save_Button = ttk.Button(settings_save_Frame, text=translate("Save"), command=lambda:save_func() if validation(settings_save_var.get()) else None)
        settings_save_Button.grid(row=7,column=0,padx=0,pady=5)
        
        stop_process_Button = ttk.Button(settings_save_Frame, text=translate("Cancel"), command=lambda:settings_save.destroy())
        stop_process_Button.grid(row=8,column=0,padx=0,pady=5)
            
        self.menu_placement(settings_save, translate("Save Current Settings"), pop_up=True)
    ...
    def pop_up_update_confirmation(self):
        ...
        if is_new_update:
            ...
            update_found_label = self.menu_title_LABEL_SET(update_confirmation_Frame, translate('Update Found'), width=15)
            update_found_label.grid(row=0,column=0,padx=0,pady=10)
            
            confirm_update_label = self.menu_sub_LABEL_SET(update_confirmation_Frame, translate('Are you sure you want to continue?')+"\n\n"+translate('The application will need to be restarted.\n'), font_size=FONT_SIZE_3)
            confirm_update_label.grid(row=1,column=0,padx=0,pady=5)
                    
            yes_button = ttk.Button(update_confirmation_Frame, text=translate('Yes'), command=update_type)
            yes_button.grid(row=2,column=0,padx=0,pady=5)
            
            no_button = ttk.Button(update_confirmation_Frame, text=translate('No'), command=lambda:(update_confirmation_win.destroy()))
            no_button.grid(row=3,column=0,padx=0,pady=5)
            
            if is_windows:
                download_outside_application_button = ttk.Checkbutton(update_confirmation_Frame, variable=is_download_in_app_var, text=translate('Download Update in Application'))
                download_outside_application_button.grid(row=4,column=0,padx=0,pady=5)

            self.menu_placement(update_confirmation_win, translate("Confirm Update"), pop_up=True)
    ...
    def pop_up_user_code_input(self):
        ...
        user_code_title_Label = self.menu_title_LABEL_SET(user_code_Frame, translate('User Download Codes'), width=20)
        user_code_title_Label.grid(row=0,column=0,padx=0,pady=5)    
        
        user_code_Label = self.menu_sub_LABEL_SET(user_code_Frame, translate('Download Code'))
        ...
        user_code_confrim_Button = ttk.Button(user_code_Frame, text=translate('Confirm'), command=lambda:self.download_validate_code(confirm=True))
        user_code_confrim_Button.grid(row=4,column=0,padx=0,pady=5)
        
        user_code_cancel_Button = ttk.Button(user_code_Frame, text=translate('Cancel'), command=lambda:self.user_code.destroy())
        user_code_cancel_Button.grid(row=5,column=0,padx=0,pady=5)
        
        support_title_Label = self.menu_title_LABEL_SET(user_code_Frame, text=translate('Support UVR'), width=20)
        support_title_Label.grid(row=6,column=0,padx=0,pady=5)    
        
        support_sub_Label = tk.Label(user_code_Frame, text=translate("Obtain codes by visiting one of the following links below." +\
        ...
        uvr_patreon_Button = ttk.Button(user_code_Frame, text=translate('UVR Patreon Link'), command=lambda:webbrowser.open_new_tab(DONATE_LINK_PATREON))
        uvr_patreon_Button.grid(row=8,column=0,padx=0,pady=5)
        
        bmac_patreon_Button=ttk.Button(user_code_Frame, text=translate('UVR \"Buy Me a Coffee\" Link'), command=lambda:webbrowser.open_new_tab(DONATE_LINK_BMAC))
        bmac_patreon_Button.grid(row=9,column=0,padx=0,pady=5)
        
        self.menu_placement(self.user_code, translate("Input Code"), pop_up=True)
    ...
    def pop_up_mdx_model(self, mdx_model_hash, model_path):
        ...
        try:
            ...
            if model_path.endswith(CKPT):
                ...
                print(translate('model_params: '), model_params)
            ...
            self.error_log_var.set("{}".format(error_text(translate('MDX-Net Model Settings'), e)))
        ...
        if is_compatible_model:
            ...
            mdx_model_set_title = self.menu_title_LABEL_SET(mdx_model_set_Frame, translate("Specify MDX-Net Model Parameters"))
            mdx_model_set_title.grid(row=0,column=0,padx=0,pady=15)
                    
            set_stem_name_Label = self.menu_sub_LABEL_SET(mdx_model_set_Frame, translate('Primary Stem'))
            ...
            mdx_dim_f_set_Label = self.menu_sub_LABEL_SET(mdx_model_set_Frame, translate('(Leave this setting as is if you are unsure.)'))
            ...
            mdx_dim_f_set_Label = self.menu_sub_LABEL_SET(mdx_model_set_Frame, translate('Dim_f'))
            ...
            mdx_n_fft_scale_set_Label = self.menu_sub_LABEL_SET(mdx_model_set_Frame, translate('N_FFT Scale'))
            ...
            mdx_compensate_Label = self.menu_sub_LABEL_SET(mdx_model_set_Frame, translate('Volume Compensation'))
            ...
            mdx_param_set_Button = ttk.Button(mdx_model_set_Frame, text=translate("Confirm"), command=lambda:pull_data())
            mdx_param_set_Button.grid(row=16,column=0,padx=0,pady=10)
            
            stop_process_Button = ttk.Button(mdx_model_set_Frame, text=translate("Cancel"), command=lambda:cancel())
            ...
            self.menu_placement(mdx_model_set, translate("Specify Parameters"), pop_up=True)
    ...
    def pop_up_vr_param(self, vr_model_hash):
        ...
        vr_param_title_title = self.menu_title_LABEL_SET(vr_param_Frame,translate( "Specify VR Model Parameters"), width=25)
        vr_param_title_title.grid(row=0,column=0,padx=0,pady=0)
                
        vr_model_stem_Label = self.menu_sub_LABEL_SET(vr_param_Frame, translate( 'Primary Stem'))
        ...
        vr_model_param_Label = self.menu_sub_LABEL_SET(vr_param_Frame, translate( 'Select Model Param'))
        ...
        vr_param_confrim_Button = ttk.Button(vr_param_Frame, text=translate('Confirm'), command=lambda:pull_data())
        vr_param_confrim_Button.grid(row=5,column=0,padx=0,pady=5)
        
        vr_param_cancel_Button = ttk.Button(vr_param_Frame, text=translate('Cancel'), command=cancel)
        ...
        self.menu_placement(vr_param_menu, translate("Choose Model Param"), pop_up=True)
    ...
    def pop_up_save_ensemble(self):
        ...
        if len(self.ensemble_listbox_get_all_selected_models()) <= 1:
            ensemble_save_title = self.menu_title_LABEL_SET(ensemble_save_Frame, translate("Not Enough Models"), width=20)
            ensemble_save_title.grid(row=1,column=0,padx=0,pady=0)
            
            ensemble_save_title = self.menu_sub_LABEL_SET(ensemble_save_Frame, translate("You must select 2 or more models to save an ensemble."))
            ensemble_save_title.grid(row=2,column=0,padx=0,pady=5)
            
            stop_process_Button = ttk.Button(ensemble_save_Frame, text=translate(translate("OK")), command=lambda:ensemble_save.destroy())
            stop_process_Button.grid(row=3,column=0,padx=0,pady=10)
        else:
            ensemble_save_title = self.menu_title_LABEL_SET(ensemble_save_Frame, translate("Save Current Ensemble"))
            ensemble_save_title.grid(row=2,column=0,padx=0,pady=0)
            
            ensemble_name_Label = self.menu_sub_LABEL_SET(ensemble_save_Frame,translate( 'Ensemble Name'))
            ensemble_name_Label.grid(row=3,column=0,padx=0,pady=5)
            ensemble_name_Entry = ttk.Entry(ensemble_save_Frame, textvariable=ensemble_save_var, justify=translate('center'), width=25)
            ...
            mdx_param_set_Button = ttk.Button(ensemble_save_Frame, text=translate("Save"), command=lambda:save_func() if validation(ensemble_save_var.get()) else None)
            mdx_param_set_Button.grid(row=7,column=0,padx=0,pady=5)
            
            stop_process_Button = ttk.Button(ensemble_save_Frame, text=translate("Cancel"), command=lambda:ensemble_save.destroy())
            stop_process_Button.grid(row=8,column=0,padx=0,pady=5)
    ...
    def online_data_refresh(self, user_refresh=True, confirmation_box=False, refresh_list_Button=False):
        ...
        def online_check():
            ...
            self.app_update_status_Text_var.set(translate('Loading version information...'))
            self.app_update_button_Text_var.set(translate('Check for Updates'))
            ...
            try:
                ...
                if refresh_list_Button:
                    self.download_progress_info_var.set(translate('Download List Refreshed!'))
                ...
                if self.lastest_version == current_patch:
                    ...
                    if is_beta_version:
                        ...
                        self.app_update_button_Text_var.set(translate('Click Here to Roll Back'))
                    else:
                        self.app_update_status_Text_var.set(f"Update Found: {self.lastest_version}")
                        self.app_update_button_Text_var.set(translate('Click Here to Update'))
                ...
                self.error_log_var.set(error_text(translate('Online Data Refresh'), e))
    ...
    def offline_state_set(self):
        ...
        self.app_update_status_Text_var.set(translate(f'Version Status: {NO_CONNECTION}'))
        self.download_progress_info_var.set(NO_CONNECTION) 
        self.app_update_button_Text_var.set(translate('Refresh'))
    ...
    def download_validate_code(self, confirm=False):
        ...
        if confirm:
            if not self.decoded_vip_link == NO_CODE:
                self.download_progress_info_var.set(translate('VIP Models Added!') )
                self.user_code.destroy()
            else:
                self.download_progress_info_var.set(translate('Incorrect Code') )
                self.user_code_validation_var.set(translate('Code Incorrect'))
    ...
    def update_stem_checkbox_labels(self, selection, demucs=False, disable_boxes=False, is_disable_demucs_boxes=True):
        ...
        for primary_stem, secondary_stem in STEM_PAIR_MAPPER.items():
            if selection == primary_stem:
                stem_text[0].set(translate(f"{primary_stem}"))
                stem_text[1].set(translate(f"{secondary_stem}"))
        ...
        if self.thread_check(self.active_processing_thread):
            ...
            if confirm:
                try:
                    ...
                    self.command_Text.write(translate('\n\nProcess stopped by user.'))
        else:
    ...
    def process_tool_start(self):
        ...
        linshi = reanslate("Time Elapsed: ")
        time_elapsed = lambda:f'{reanslate} {time.strftime("%H:%M:%S", time.gmtime(int(time.perf_counter() - stime)))}'
        ...
        try:
            ...
            if self.chosen_audio_tool_var.get() == MANUAL_ENSEMBLE:
                ...
                multiple_files = True
                if total_files <= 1:
                    self.command_Text.write(translate("Not enough files to process.\n"))
                    self.process_end()
                    return
            if self.chosen_audio_tool_var.get() == ALIGN_INPUTS:
                ...
                if not total_files == 2:
                    self.command_Text.write(translate("You must select exactly 2 inputs!\n"))
                    self.process_end()
                    return
            ...
            for file_num, audio_file in enumerate(inputPaths, start=1):
                ...
                command_Text(translate('Process starting... ')) if not audio_tool.audio_tool == ALIGN_INPUTS else None
                ...
                if audio_tool.audio_tool == ALIGN_INPUTS:
                    command_Text(translate('Process starting... \n'))
